...
pub fn run_docopt() -> Result<(), TrailerError> {
    ...
    if args.get_bool("bin") || args.get_bool("bit") {
        ...
        let config = {
            if args.get_bool("bin") {
                if let Some(config) = conf.binance {
                    ::exchanges::binance::connect(&config.api_key, &config.secret_key)?
                } else {
                    return Err(TrailerError {
                        error_type: TrailerErrorType::ConfigError,
                        message: "could not find binance keys in .config.toml".into(),
                    })};
                }
            } else {
        ...
        let client = match conf.clone().binance {
            Some(config) => {
                if args.get_bool("bin") {
                    ::exchanges::binance::connect(&config.api_key, &config.secret_key);
                } else if args.get_bool("bit") {
                    ::exchanges::binance::connect(&config.api_key, &config.secret_key);
                } else { return Err(TrailerError {
                    error_type: TrailerErrorType::ConfigError,
